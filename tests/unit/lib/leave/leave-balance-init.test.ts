...
describe('Leave Balance Initialization', () => {
  ...
  describe('initializeUserLeaveBalances', () => {
    ...
    it('should create leave balances for eligible leave types', async () => {
      ...
      const result = await initializeUserLeaveBalances(memberId, currentYear, tenantId);

      expect(result.created).toBe(2);
      expect(result.skipped).toBe(0);
      expect(mockPrisma.leaveBalance.createMany).toHaveBeenCalledWith({
        data: expect.arrayContaining([
          expect.objectContaining({ memberId, leaveTypeId: 'lt-1', year: currentYear, tenantId }),
          expect.objectContaining({ memberId, leaveTypeId: 'lt-2', year: currentYear, tenantId }),
        ]),
        skipDuplicates: true,
      });
    });
    ...
    it('should calculate 28 days for employees with 5+ years service', async () => {
      ...
      await initializeUserLeaveBalances(memberId, currentYear, tenantId);
    ...
    it('should get tenantId from member if not provided', async () => {
      (mockPrisma.teamMember.findUnique as jest.Mock).mockResolvedValue({
        dateOfJoining: new Date('2020-01-01'),
        gender: 'MALE',
        tenantId: 'auto-tenant-id',
      });

      (mockPrisma.leaveType.findMany as jest.Mock).mockResolvedValue([]);
      (mockPrisma.leaveBalance.findMany as jest.Mock).mockResolvedValue([]);

      await initializeUserLeaveBalances(memberId, currentYear); // No tenantId provided
  ...
  describe('reinitializeUserLeaveBalances', () => {
    ...
    it('should update entitlement if changed and no leave used', async () => {
      ...
      const result = await reinitializeUserLeaveBalances(memberId, currentYear, tenantId);
  ...
  describe('initializeAllUsersLeaveBalances', () => {
    it('should process all active employees', async () => {
      ...
      // Mock for each member's initialization
      (mockPrisma.teamMember.findUnique as jest.Mock).mockResolvedValue({
        dateOfJoining: new Date('2020-01-01'),
        gender: 'MALE',
        tenantId: 'tenant-1',
      ...
      const result = await initializeAllUsersLeaveBalances('tenant-1');

      expect(result.membersProcessed).toBe(3);
      expect(result.totalCreated).toBe(3); // 1 balance per member
    ...
    it('should aggregate created and skipped counts across all members', async () => {
      (mockPrisma.teamMember.findMany as jest.Mock).mockResolvedValue([
        { id: 'member-1' },
        { id: 'member-2' },
      ]);
      ...
      expect(result.totalCreated).toBe(2); // 1 Annual Leave per member
      expect(result.totalSkipped).toBe(2); // 1 Maternity (PARENTAL) skipped per member
    });
  });
