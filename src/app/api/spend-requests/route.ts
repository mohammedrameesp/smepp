/**
 * @file route.ts
 * @description List and create spend requests
...
// GET - List spend requests
async function getSpendRequestsHandler(request: NextRequest, context: APIContext) {
    ...
    const [total, requests] = await Promise.all([
      db.spendRequest.count({ where }),
      db.spendRequest.findMany({
        where,
        include: {
          requester: {
            select: {
...
async function createSpendRequestHandler(request: NextRequest, context: APIContext) {
    const { tenant, prisma: tenantPrisma } = context;
    if (!tenant?.tenantId) {
      return NextResponse.json({ error: 'Tenant context required' }, { status: 403 });
    }
    ...
    const validation = createSpendRequestSchema.safeParse(body);
    ...
    const referenceNumber = await generateSpendRequestNumber(prisma, tenantId);
    ...
    const {
      ...
      totalMonthly,
      totalContractValue,
    } = await calculateSpendRequestItems(data.items, formCurrency, isSubscriptionType, tenantId);

    // Create purchase request with items
    const spendRequest = await db.spendRequest.create({
      data: {
        referenceNumber,
        requesterId: userId,
        title: data.title,
    ...
    await logAction(
      tenantId,
      userId,
      ActivityActions.SPEND_REQUEST_CREATED,
      'SpendRequest',
      spendRequest.id,
      {
        referenceNumber,
    ...
    return NextResponse.json(spendRequest, { status: 201 });
}
