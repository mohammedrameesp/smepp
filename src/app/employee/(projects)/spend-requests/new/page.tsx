  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to create spend request');
      }

      const data = await response.json();
      router.push(`/employee/spend-requests/${data.id}`);
    } catch (error) {
      console.error('Error creating spend request:', error);
      setError(error instanceof Error ? error.message : 'Failed to create spend request');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Card className="mb-6">
            <CardHeader>
              ...
              <CardDescription>Basic information about your spend request</CardDescription>
            </CardHeader>
