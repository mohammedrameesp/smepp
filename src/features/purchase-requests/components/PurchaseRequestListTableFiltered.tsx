/**
 * @file PurchaseRequestListTableFiltered.tsx
 * @description Table component with client-side filtering for purchase requests
...
interface PurchaseRequestListTableFilteredProps {
  requests: PurchaseRequestItem[];
  isAdmin?: boolean;
}

export function PurchaseRequestListTableFiltered({
  requests,
  isAdmin = false
}: PurchaseRequestListTableFilteredProps) {
  ...
  const {
    ...
    setFilter,
    resetFilters,
  } = useClientDataTable<PurchaseRequestItem, PurchaseRequestFilters>({
    data: requests,
  ...
  const handleFilterChange = <K extends keyof PurchaseRequestFilters>(key: K, value: PurchaseRequestFilters[K]) => {
    setFilter(key, value);
    setPage(1);
  };
