/**
 * @file spend-request-creation.ts
 * @description Spend request creation utilities - item calculations and notifications
 * @module domains/projects/spend-requests
 *
 * PURPOSE:
 * Provides reusable functions for creating spend requests.
...
export interface SpendRequestItemInput {
  description: string;
  quantity: number;
  unitPrice: number;
  currency: string;
...
export interface SpendRequestNotificationParams {
  tenantId: string;
  userId: string;
  spendRequest: {
    id: string;
...
/**
 * Calculate totals and QAR conversions for spend request items
 *
 ...
 *
 * @example
 * const result = await calculateSpendRequestItems(items, 'USD', true, 'tenant-123');
 * // result.totalAmountQAR contains total in QAR
 * // result.items contains items ready for database
 */
export async function calculateSpendRequestItems(
  items: SpendRequestItemInput[],
  formCurrency: string,
...
/**
 * Send notifications for a newly created spend request
 * Checks for approval policy and notifies appropriate approvers
 *
 * @param params - Notification parameters
 *
 * @example
 * await sendSpendRequestNotifications({
 *   tenantId: 'org_123',
 *   userId: 'user_456',
 *   spendRequest: { id: 'sr_789', currency: 'QAR' },
 ...
 * });
 */
export async function sendSpendRequestNotifications(
  params: SpendRequestNotificationParams
): Promise<void> {
  const {
    tenantId,
    userId,
  ...
  const approvalPolicy = await findApplicablePolicy('SPEND_REQUEST', { amount: totalAmountQAR, tenantId });

  if (approvalPolicy && approvalPolicy.levels.length > 0) {
    // Initialize approval chain (pass userId to check manager relationship)
    const steps = await initializeApprovalChain('SPEND_REQUEST', spendRequest.id, approvalPolicy, tenantId, userId);
    ...
    if (firstStep) {
      ...
      if (approvers.length > 0) {
        const emailContent = spendRequestSubmittedEmail({
          referenceNumber,
      ...
      await createBulkNotifications(
        approvers.map(approver => ({
          ...
          title: 'Spend Request Approval Required',
          message: `${userName} submitted a spend request (${referenceNumber}) for ${spendRequest.currency} ${formatNumber(totalAmount)}. Your approval is required.`,
          link: `/admin/spend-requests/${spendRequest.id}`,
    ...
    if (admins.length > 0) {
      ...
      const notifications = admins.map(admin =>
        NotificationTemplates.spendRequestSubmitted(
          admin.id,
          referenceNumber,
          userName,
          title,
