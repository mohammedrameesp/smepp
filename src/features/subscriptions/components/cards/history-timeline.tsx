 ...
 * @description Visual timeline of all subscription changes and actions
 * @module components/domains/operations/subscriptions
 *
 * Features:
 * - Chronological display of all subscription history events
 * - Color-coded action badges (created, cancelled, reactivated, reassigned, etc.)
 * - Status transition tracking (old status → new status)
 * - Member assignment changes with names
 * - Renewal date changes tracking
 * - Performer attribution (who made each change)
 * - Notes and context for each action
 * - Timestamp display with formatted dates
 *
 * Props:
 * @param history - Array of history entries with performer details
 * @param purchaseDate - Original purchase date (optional, for context)
 * @param cancelledAt - Cancellation date (optional, for context)
 * @param reactivatedAt - Reactivation date (optional, for context)
 *
 * Usage:
 * ```tsx
 * <HistoryTimeline
 ...
 * - CREATED: Initial subscription creation
 * - CANCELLED: Subscription cancellation
 * - REACTIVATED: Restoration after cancellation
 * - REASSIGNED: Member assignment changes
 * - RENEWED: Renewal date updates
 * - STATUS_CHANGED: Manual status changes
 *
 * Badge Colors:
 * - Green: Reactivated
 * - Red: Cancelled
 * - Blue: Reassigned
 * - Default: Created, Renewed
 */
'use client';
